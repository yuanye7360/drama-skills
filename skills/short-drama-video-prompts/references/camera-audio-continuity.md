...
对白的可信度不只取决于说什么，还取决于怎么说。以下几条是 delivery 层的通用规则：

- **节奏写在文本里，照它执行**：剧本台词本身的断句、逗号、省略号已经暗示了节奏；delivery 忠实执行这份节奏就够了，快不等于含糊——语速快的角色仍要保证关键词清楚可辨。
- **抢话是正常的，但关键词要留清楚**：两人对话时台词尾音被对方抢话打断是真实对话的常态；delivery 可以描述这种重叠，但每句里真正承载信息的关键词（称呼、否定词、决定性动词）不能被吞没在重叠里，否则观众丢失关键信息。
- **停顿是事件，不是空洞**：合法的停顿里必须发生一件事——一次评估、一次决定、一次拒绝回答；写「停顿」时同时写清停顿里在发生什么（呼应 `performance-action-timing.md`「Listening and reaction」第 2、3 条标记），单纯的静默没有内容支撑，读起来只是卡顿。
- **真实说话有「杂质」**：打断、没听清的半句、重复、没说完的句子——这些「杂质」是活人说话的证据；把每句台词都写得语法完整、逻辑严密，反而破坏了街头感和真实感。这与下面的音量对比规则同属「越完美越假」的同一类问题，只是作用的层面不同（一个管音量，一个管句法完整度）。**这条只描述表演层的处理方式（停顿、重复的语气），不能用来改写已接受的逐字台词文本**——真正需要台词本身带口语杂质，要请求 write owner 修改剧本，而不是在 delivery 里擅自增删字词。
...
- **`craft_default`**：越关键或越具威胁性的台词，越常见的可信写法是压低音量、放慢速度，而不是喊出来——压低反而让对方和观众更紧张；把每一次情绪高点都写成提高音量，是最容易读出来的机械腔调。这条描述常见分布，不是禁止喊出来，剧本/agenda 要求嘶吼时仍应如实写。
