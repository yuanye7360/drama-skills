...
| VID-21 | craft_default | Lens character is written as diagonal field of view plus physical camera distance plus observable optical outcome; millimetres, aperture, sensitivity, and lens model names are not the primary control because a generation surface resolves them to a comfortable middle value. Where the creator profile declares equipment vocabulary, that wording is reused verbatim and the suite invents none. |
| VID-22 | reviewed_invariant | Lens character is chosen by content type rather than by emotion, and one beat does not mix content classes—face portrait, environmental geography, and macro detail together force a compromise value that satisfies none. Different classes need declared cuts, each with its own lens character. |
| VID-23 | reviewed_invariant | A shot that must open on its subjects declares first-frame occupancy; an empty opening exists only on explicit creator request, and a flash cut still carries the required subject or location information immediately. |
| VID-24 | reviewed_invariant | Where a shot carries contact, load bearing, throwing, liquid, particles, or vehicles, it states the physical properties and their cause and effect, and describes action as an already-established state rather than the process of getting there. |
| VID-25 | reviewed_invariant | Lighting is a priority constraint, not decoration: the shot names key source, direction, which side of the light the camera sits on, which side of the subject is shadow or rim, background brightness, and exposure priority. A backlit shot states explicitly that exposure serves the backlight rather than the face. |
| VID-26 | reviewed_invariant | Per-shot performance projects the accepted performance master profile: constant core preserved, behaviours impossible in this shot converted rather than deleted, only characters actually in frame written, and no behaviour invented beyond profile and screenplay. |
| VID-27 | craft_default | Where a shot carries performance change, it states how eye life shifts with the beat—blink rate, gaze steadiness, catchlight—and staggers ensemble reactions instead of giving the group one simultaneous response. |
| VID-28 | reviewed_invariant | When a multi-panel board is fed as a generation reference, the text separates what each reference controls—identity reference for face, build, wardrobe, and unique marks; board reference for action order, shot rhythm, body movement, composition, movement direction, camera angle, and visual progression—states that each panel is a consecutive keyframe in panel order, and carries both complementary negations: the output is one continuous frame with no grid, borders, labels, arrows, or annotation text, and the board is not to be read as a single image. The finished medium is named so the board's drawing medium is not inherited as a style requirement, and the intensity progression is restated here because a reference image carries no temporal intent. Such a generation is a multi-shot container and is billed as one. |
| VID-29 | craft_default | Where movement quality carries the performance, write it as observable opposed poles—weight strong-grounded or light, time quick or sustained, space direct or indirect, flow bound or free—naming which pole this beat takes, rather than as adverbs. This sits above the physics lock: physics answers whether the motion is possible, quality answers what it feels like, and each pole has a visible bodily consequence an adverb does not. Only the one or two dimensions that actually change from the previous beat are written. |
