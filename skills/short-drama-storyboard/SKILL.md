...
要做多格故事板时读 [board-sheet.md](references/board-sheet.md)，并先分清是哪一种：

- **预览板**（在镜头表之前）：用故意不同于成片的作画媒介（粗铅笔、黑白、未完成）提案
  动作编舞与镜头进展，带宣告过的标注色彩图例与跨格进展轴。它是提案与项目工作历史，
  标注只活在板层、永不进入交付帧，也不构成镜头数公式（`SHT-25`）。
- **关键帧板**（在镜头表之后）：用已接受视觉方向渲染若干已接受关键帧，每格绑定一个
  已接受边界，买到的是跨格光色与身份一致性。它**不是首帧来源**：交给视频生成的首帧仍
  逐镜渲染，裁下来的一格不是那张帧；格数跟随已选定的镜数（`SHT-26`）。作为
  `keyframe-prompts.md` 里的一节，不新增产物。
