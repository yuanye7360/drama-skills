...
创作者想用一次生成换取**跨镜一致性与轨迹**——把六到九个关键帧画进同一张多格板图——时读
[board-sheet.md](references/board-sheet.md)。板图作为 `keyframe-prompts.md` 里的一节，
不新增产物；它是一致性锚与比稿用，**不是首帧来源**：交给视频生成的首帧仍逐镜渲染，
裁下来的一格不是那张帧。格数跟随已经选定的镜数，版式不决定镜头数（`SHT-25`）。
