...
| SHT-25 | reviewed_invariant | A previz board proposes and does not decide: its annotation legend is declared, its per-panel note strip sits outside the artwork, and it may propose a shot progression without becoming a shot-count formula—coverage and the shot list still decide the shots. When the board is fed as a generation reference rather than read by a person, the same boundary moves into the prompt: the text states positively that the output is one continuous frame with no panel grid, borders, labels, arrows, or annotation text, and names the finished medium so the board's sketch medium is not inherited as a style requirement. A board-referenced generation covering several accepted shots is a multi-shot container and is billed as one, cuts landing only on accepted shot boundaries. |
| SHT-26 | reviewed_invariant | A keyframe board renders several accepted keyframes: each panel binds one accepted shot boundary and panel order follows source order. It is a consistency and comparison anchor, not a start-frame authority—a frame handed to video generation is still rendered per shot, a cropped panel is not that frame, and panel count follows the shots already chosen. |
