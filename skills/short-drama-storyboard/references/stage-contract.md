...
| SHT-23 | craft_default | When several characters share a sheet, or a monochrome surface makes them hard to tell apart, each character carries a fixed recognition anchor (tonal value plus silhouette features) reused across panels. The anchor governs how the same person is recognised and never rewrites the identity facts owned by assets. |
...
| SHT-27 | craft_default | Each panel is written with the same fixed field set — reference names, shot size, camera movement, action/composition, dynamic elements, environment, effects, shot note, annotations — so panels stay comparable across a sheet. |
