...
| SHT-21 | reviewed_invariant | A storyboard sheet is a previs layer between shots and keyframes: it owns panel composition sketches, staging/camera/action intent, light direction, panel-to-shot mapping and panel order, and owns no shot purpose/duration/boundary, no identity or geography fact, and no final look. Each panel binds exactly one shot in `shots.jsonl` and each shot appears in at most one panel; panel order follows the narrative order already fixed upstream, and a panel projects only that shot's start beat. |
| SHT-22 | craft_default | Sheet artwork stays a rough black-and-white planning drawing decoupled from the project's final look, with intent carried by a colour annotation overlay (body motion, camera motion, composition, light direction, VFX, shot notes). Annotations visualise the accepted shot record rather than restating or overriding its framing and camera position. |
| SHT-23 | craft_default | When several characters share a sheet, or a monochrome surface makes them hard to tell apart, each character carries a fixed recognition anchor (tonal value plus silhouette features) reused across panels. Anchors cover only the characters that actually appear on that sheet and are stated once, after the reference binding and before the panels, so the panels can be read against them. The anchor governs how the same person is recognised and never rewrites the identity facts owned by assets. |
| SHT-24 | reviewed_invariant | Panels keep one subject one instance, one frozen moment per character, exact counts for countable content, one stable spatial relationship across panels, and one screen direction; shot size follows the pressure change the scene is making rather than an even distribution. |
| SHT-25 | craft_default | Panel descriptions lock composition through quantified relationships rather than vague magnitude words, and those relationships are written out per panel rather than compressed away. |
| SHT-26 | taste_option | A sheet may add an overhead scene diagram and a character sketch strip beneath the panels to pin spatial and identity locks onto the same page; ordinary short scenes omit this band. |
| SHT-27 | craft_default | Panels stay comparable through one shared shape — a heading carrying shot size and camera movement, a prose body carrying the frozen moment, and one annotation line — while what belongs to the whole sheet (colour legend, camera vocabulary, element progression, exclusions, recognition anchors) is stated once globally instead of restated per panel. What does not apply to a panel is omitted rather than filled with a placeholder, and a panel's annotation line lists only the arrows that panel actually carries. |
