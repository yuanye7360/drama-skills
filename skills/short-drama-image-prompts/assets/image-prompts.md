...
> <自然语言正文，语言跟随 `short-drama.json#/format/prompt_language`（默认 `en`），不跟元信息的 `#/language`。**风格锁在最前**（已接受制作形态/视觉方向，描述性、不指名 IP），随后用途/主体与区分性锚点，再状态、构图、空间/尺度、材质/光线、背景、文字政策、保持/排除依次展开。不要出现字段名、hash、审查话术或生成历史。>
