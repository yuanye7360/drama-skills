...
**每条提示词起首先写风格锁**（见下「风格锁」小节），把画风框定后再进以下八段。

### 风格锁（每条提示词起首）

**每条资产提示词——人物、场景、道具、变体——都在正文起首写一段风格锁**，把画风框定后再进
八段配方。风格锁投影项目已接受的 `creator_authority/{visual_direction,production_profile}`：

- **内容**：画风类别 + 线条/表面处理 + 材质对光的响应 + 负面词（压掉不属于该形态的质感）。
  例：「日式手绘赛璐璐动画画风，柔和干净的线条、圆润造型，水彩淡彩背景与自然暖调；排除照片
  写实、厚涂 3D、数码硬边、胶片颗粒」。
- **描述性、不指名 IP**：把画风拆成可执行的视觉特征，**绝不写真实工作室/画师/艺术家/品牌/IP
  的名字**（不写「宫崎骏 / 吉卜力 / 某某画师风格」，改写它由哪些线条、造型、上色、色调构成）。
  两个理由：① 项目 NON-IP 契约；② 图模型碰到指名艺术家常降质或直接拒绝。
- **全项目逐字复用**：同一形态的风格锁写死一次、每条提示词逐字复用（一处修改全项目更新），
  不要每条各飘一版；等同于把已接受视觉方向导出的固定前缀。
- **起首但不越权**（`IMG-02` 的澄清）：风格锁只定画风（形状语言/线条/表面/材质光/负面词），
  **不改身份、地理、尺度、持物、可读文字与故事状态**；识别锚点紧跟其后。IMG-02 的「识别优先」
  指风格词不能*替代*身份事实，不是风格不能*打头*——形态锁是结构约束，不是「电影感」这类空泛审美词。
- **`visual_direction` 为 `unset` 时不编**：先让创作者定画风，不用默认审美或凭对话记忆补造。
...
```text
[风格锁：画风类别 + 线条/表面 + 材质光响应 + 负面词，描述性、不指名 IP]。
...
- `character_sheet`：身份锚点绑定一个造型版本，默认输出三视图（头肩特写｜正面全身·头裁出画面｜背面全身），只在 `reuse_job` 确实只需单面时降级；没有文字承载面时删除文字字段。
...
- **`craft_default`**：先建立清楚，再添加风格。这里的「风格」指*空泛审美形容词*（电影感、高级、氛围感）——后置且要落到可观察选择；**起首的风格锁（已接受制作形态）不在此列，形态是结构约束，照常打头**。参考板需要可比较性时，均匀光常优于戏剧光；但这是用途相关默认，不是硬规则。
