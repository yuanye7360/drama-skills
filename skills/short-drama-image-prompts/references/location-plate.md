...
- **`craft_default`**：默认选择斜侧（约 3/4）观察角度，而非正面平拍。

  **为什么正面平拍会出问题**：location plate 的作用是让后续许多镜头在不同机位下都能共享同一份空间记忆，而不必每次重新推导房间格局。正对拍摄的镜头会把画面里朝向镜头的所有表面压缩到近乎同一个隐含距离上——真正能告诉模型“这面墙比那个柜台远五米”的线索，是近大远小、遮挡关系和透视线收敛，而这些线索在一张完全正面的画面里几乎都不存在。它更像一张摆正的立面图案，而不是一张有纵深的照片。
  这会带来两个连锁问题：一是这张参照图本身没有深度线索，后续镜头若要求“从检修台后面看过去”这类新角度，模型没有任何依据推算墙后是什么样子，只能自己发明，而这份发明很可能和之后正式确定的地理设定互相矛盾；二是画面里“在……前面/后面”这类相对位置描述，缺少一个纵深参照系可以落地——本该站在镜头和柜台之间的人物，可能被画成站在柜台后面，因为画面里根本没有表达“前后”的视觉证据。
  斜侧角度能同时保留至少两个不平行的面（例如一面墙向纵深方向退去、另一道墙或柜台横向切过画面），恰好补上正面平拍缺失的透视收敛和遮挡关系——近处物体挡住远处物体、纵深方向的线条向画面深处收拢，深度顺序因此变得可读。附带的好处是：因为斜侧角度已经同时展示了空间的两个面，它实际能覆盖后续正反打镜头需要参照的近乎一整圈角度范围，而一张纯正面的图只能支持贴着那条正对轴线附近的机位。

  **好写法（斜侧，两组面同时可读）**：
  > 镜头略偏离正对通道的中线，从入口斜侧方向向内看：右侧检修台沿墙斜向延伸向纵深，左侧高窗随透视线逐渐收窄，两者在画面里形成明显的远近关系；地面接缝线朝画面深处收拢，标示出可通行走道的方向。

  **不好写法（正面平拍，读成壁纸）**：
  > 镜头正对通道中线拍摄，检修台、窗户和后墙在同一个平面上并排展示。——所有物体隐含处于同一深度，模型没有近大远小或遮挡关系可以参照；之后若要求“从检修台后面看过去”，模型只能自己发明后墙外的样子，很可能和已确定的地理设定冲突。

  仅当创作者明确要求正面对称构图（例如仪式化、舞台感场景）时才选正面平拍，并单独确认它不会引入未声明背景——这类场景通常本来就不需要后续从多个角度拍摄同一空间，所以缺深度线索的代价较小。
