...
- **`craft_default`**：远超人体尺度的巨大道具或生物，每一次生成都要带一个中性人形比例参照，不能只在第一张图给一次就假设后续沿用。

  **为什么尺度会在连续生成里悄悄缩回人类身高**：模型见过的“某种道具/生物”的训练画面，绝大多数都是这个东西处于寻常、符合人体直觉尺度的画面——巨大到反常的尺度是训练分布里远比寻常尺度稀有的画面模式。当一条提示词只用文字声称“这个东西极其巨大”，却没有在同一张画面里放进任何一个模型已经“知道”真实尺寸的参照物（人、门、常见家具），画面里就没有任何视觉证据能供模型校准大小，它最容易依赖的仍是自己最强的先验——寻常尺度。更关键的是：同一件资产的每一次重新生成，都是一次独立的抽样，不是接着上一张图继续画，模型不会把“昨天那张图判定这东西有多大”当成今天生成时的既定事实延续下去——如果这一次生成的画面里没有重新提供尺度证据，它就会重新向训练分布里更常见的寻常尺度漂移，即使上一张图的比例是对的。
  这正是为什么比例参照必须**每次都出现在同一张画面里**，而不是只在资产创建时确立一次：每一次新的 plate/variant/状态生成，都是模型的先验和这次画面证据之间的一次新博弈，只有当次画面里确实存在一个可比较的参照物，才能把博弈引向正确的尺度。
  参照仍不得引入剧情角色身份（不用具名角色，也不用可能被误认成角色的具体特征），因为这会让道具板暗示“这个角色出现在这件道具旁边”，越过了道具板本该只交代道具本身的职责；用一个无脸的中性人形剪影，或者一根标注尺度的比例标尺，同样能提供校准所需的视觉证据，且不带入身份、选角或剧情含义——这和“不要用剧情角色的手当默认参照”是同一条道理的两个应用场景，并不冲突。

  **好写法（每次都带比例参照）**：
  > 巨兽站立于画面中央，肩部高度约为站在其脚边的一个无脸中性人形剪影身高的六倍；人形剪影仅用于标示比例，无面部细节、无服装身份特征，不属于剧情角色。

  **不好写法（第一张给了参照，后续默认沿用）**：
  > 第一张 prop plate 写明“高度约为一个成年人形轮廓的六倍”；同一巨兽的损伤状态变体第二张图只写“同一巨兽，右前肢多处擦伤”，未重复比例参照。——这张图独立生成时完全没有尺度证据，模型很可能把巨兽的体型悄悄画回接近人类高度，即使上一张图的比例明明是对的。
