...
- **`taste_option`**：姿态、表情范围由参考用途决定；视图数量默认三视图（3/4 头肩特写｜正面全身·头裁出画面｜背面全身），仅当 `reuse_job` 确实只需单面时才显式降级，不固定表情格。

## 3. 构图方法

### 默认三视图（character_sheet 标准输出）

`character_sheet` 默认输出三格三视图，从左到右：**3/4 头肩特写 ｜ 正面全身（头裁出画面）｜
背面全身**，三格同一角色设计、同一套 Look、跨格严格一致（版式细则、配色配比、风格锁与
无头全身理由见 [生产资产图配方](production-sheet-recipes.md) 的「三视图标准版式」）。特写格
承担脸，两张全身格承担比例与正背结构——**正面全身裁掉头**避免身体尺度上的脸漂移，背面全身
保留后脑交代发型背面。散文里必须写出跨格一致的锚点句，否则三格会各画各的。

下列单图板式作为**替代选择**：某个 `reuse_job` 确实只需其中一面时，显式降级并写明理由。

### 单图选择（替代版式）

1. **身份板**：中性全身或三分之二身为主，清楚显示比例与一套 Look；可附少量正/侧细节，但所有视图是同一时刻、同一 Look。
2. **面部识别板**：头肩为主，均匀光，发际与轮廓完整；适合妆面/身份锚点，不承担服装全貌。
3. **服装 Look 板**：全身正面与必要背面/局部，突出层次、穿法和鞋；单独成板时面部仍保持身份，不换成丢失身份的无脸 mannequin（三视图里正面全身格头裁出画面是另一回事——脸由特写格承担，不算丢身份）。
...
三视图 `character_sheet` 例外：提示词起首先立**风格锁**（形态词 + 线条/表面 + 负面词），再进上表顺序——先框风格、再框身份，见 [生产资产图配方](production-sheet-recipes.md) 的「三视图标准版式」。
