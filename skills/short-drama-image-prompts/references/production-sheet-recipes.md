...
**默认采用三视图三格版式**（3/4 头肩特写｜正面全身·头裁出画面｜背面全身，见下「三视图标准版式」），
...
- **风格锁只锁画风，不锁画面内容**：风格锁常从项目 Look Development 帧或 `visual_direction`
  取词，而 lookdev 帧**自带场景**（教室、夜空…），其风格锁尾部往往带「背景有纵深感 / 场景纵深 /
  场景光向」这类**场景描述**。提取到设定板时**必须剔除场景、背景、光向、纵深词**，只保留形态 +
  线条/表面 + 上色 + 负面词——否则起首「背景有纵深感」会和设定板「纯色干净背景」在同一条里
  自相矛盾。不要整句照抄 lookdev 的风格锁；风格锁锁的是画风，背景与光由「设定板的光」和版式另行决定。
