...
- **多格故事板图**（`SHT-25`）。新增
  `references/board-sheet.md`：把六到九个已接受关键帧画进同一张板，买到的**不是省几次
  生成，而是这几格彼此一致**——分开生成 N 张首帧，最贵的失败是它们的光向、色调、空间与
  身份互相对不上，而那是后面每段影片接不起来的根源；同一次生成天然共享这些，相邻格的
  位移还能直接表达跨镜连续。同时写明两条硬边界：**板图不是首帧来源**（一板九格每格只有
  九分之一像素，裁边会带进相邻格的分隔线；且板里坏一格重生成会让另外八格一起变，
  一致性白付），以及**格数跟随已选定的镜数，版式永不决定镜头数**——这是既有的
  「宫格只是人工比较界面」界线的第三个入口，也是最容易被绕过的一个，因为「九格刚好」
  读起来像个理由。板图作为 `keyframe-prompts.md` 里的一节，不新增产物。**记为新增**：
  不做板图的项目行为不变。
