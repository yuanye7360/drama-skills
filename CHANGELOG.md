...
- **图片提示词补齐资产锁定的因果解释与压力测试**（`short-drama-image-prompts`）：
  `character-and-look`、`common-recipe`、`location-plate`、`prop-plate`、
  `edit-and-revision`、`review-and-fixtures` 新增「去头三联身份板」的完整机制说明
  （为什么宽镜会从缩小模糊的全身小图上取错脸部特征）、正面优先于负面表述的失败原理
  （生成模型没有负向参数，NOT-stack 会把被禁止的概念反而写进画面）、60/30/10 调色比例表
  （须从已接受视觉方向或参考图推导，不能自创）、场地默认 3/4 视角的推理（正面平拍会变成
  平面壁纸，且模型会在画面外自行发明周边环境）、巨物比例锚（缺人形参照物时模型会把巨物
  悄悄缩回接近人类身高）、编辑「改多了」时应收紧 `preserve` 而非继续叠加 `changes`
  （漂移会累积，不会自行收敛），以及多姿势 + 同框一致性压力检验的完整范例。全部归
  `craft_default`，创作者可说明理由覆盖；未引入任何模型、平台或供应商专属术语。
- **分镜补齐空间与镜头的机制解释**（`short-drama-storyboard`）：`shot-craft`、
  `blocking-playbooks`、`production-shot-grammar` 新增朝向与视线分开判断的机制
  （身体朝向不等于目光方向，模型默认会把两者拉直）、场景空间地图一次锁定跨镜复用的原理
  （下游没有跨镜空间记忆，每镜重新描述地理等于每镜重新掷骰子）、弱定位词（附近/某处）
  与强锚定写法（距离…1 米内/手贴着门把）的对照、剪切类型白名单（淡入淡出/溶解一类模糊
  转场指令会让执行端自行发明过场内容）、手持镜头的物理质感写法（"手持"两字默认会变成
  逐帧随机跳动，需要写清呼吸感的规律性）、按维度展开的多镜连续性总清单（在场人物名单、
  视线目标、光线方向、服装伤势、环境附着物、道具手位、物体状态、情绪演进,每项配具体
  失守场景），以及镜头效果的视觉结果导向描述（用前景占比/背景压缩程度/景深范围代替容易
  漂移的抽象术语或参数）。全部归 `craft_default`；未引入具体焦段角度数值或任何平台专属
  参数体系。
- **视频提示词补齐表演方法论**（`short-drama-video-prompts`）：
  `performance-action-timing`、`review-and-fixtures`、`camera-audio-continuity`、
  `motion-recipe` 新增目标/障碍/手段/节拍/潜台词五要素的完整展开（目标必须是瞄准对方的
  动词而非状态词、手段随场景受挫而更换、节拍转换需要可见的行为标记）、listening/reaction
  的四个可观察标记、身体物理生活的四个参数（重心/节奏/开放度/呼吸）、Status 隐性地位的
  行为清单、Ensemble 群体反应错落与「威胁点冻结」等空间动机原则、角色一致性 Master
  Profile 的完整写作模板（体态即传记、每个 tic 配 trigger、mask 与 crack 的转折句式）
  与一则合成范例、坏演技症状对照从 8 条补齐到完整 15 条、演技 0–5 分自评量表的六个等级
  完整定义、关键台词常压低音量而非喊出的音量对比规则,以及 proxemics 四个距离带。全部归
  `craft_default`，不改动 `performance_arcs` 等既有字段结构。
