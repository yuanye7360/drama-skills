...
- **角色表演主档案成为资产阶段的跨集产物**（`设定集/performance-profiles.jsonl`，
  `AST-08`–`AST-11`）。此前套件里没有任何地方拥有「这个角色一贯怎么动、怎么说话」，
  于是每一帧和每一镜各自即兴，跨镜一致性只能靠身份锚点撑住外形，撑不住表演。新的
  `references/performance-master-profile.md` 给出主档案配方（体格承载生平、带触发条件的
  小动作、面具与裂缝、步态命名、眼神生命）与声音身份记录，并规定下游只做**改写**而非
  粘贴：恒定核心不变、这一镜做不出来的行为**转化而不删除**。其中眼神一段是必写的
  `reviewed_invariant`——静帧永远不眨眼，缺失它在文字审查与关键帧审查里都看不出来，
  会一路到成片。**记为新增**：不写主档案的项目行为不变，写了的项目获得关键帧与逐镜
  投影的统一来源。

- **多格故事板：预览板与关键帧板**（`SHT-25`、`SHT-26`）。新增
  `references/board-sheet.md`。两种板长得像、位置相反，选错整张板白做：

  **预览板**在镜头表**之前**，提案动作编舞与镜头进展。它的作画媒介**故意不同于成片**
  （粗铅笔、黑白、未完成）——这不是省事：注意力因此落在动量与轮廓可读性而不是材质，
  媒介本身就声明了"这不是 look、也不是首帧"，而且 12 格的小面积下粗线条是唯一还读得出
  动作的画法。它带两个此前套件完全没有的装置：**标注色彩系统**（红＝身体运动、蓝＝相机
  运动、绿＝构图、橙＝光向、黄＝元素 VFX、黑＝镜头笔记，图例必须宣告且一经声明就固定），
  以及**进展轴**——把升级写成跨格曲线（早／中／晚／终各自的强度），而不是逐格堆形容词：
  十二个绝对值里只要两个互相矛盾，整板的升级感就塌了。标注**只活在板层**，永不进入交付
  帧，否则箭头、标签与面板编号会被执行端当成应该画出来的图形与字符；板上的红箭头在帧里
  改写成可测量的走位与朝向。预览板还是套件里少数允许一小段负面清单的交付文本
  （无时间戳／无对话／无额外角色／无水印），因为它不是交付帧、而这些正是生成端会主动
  添加的东西。

  **关键帧板**在镜头表**之后**，用已接受视觉方向渲染若干已接受关键帧，每格绑定一个已接受
  边界。它买到的不是省几次生成，而是这几格彼此一致——分开生成 N 张首帧，最贵的失败是
  它们的光向、色调、空间与身份互相对不上，而那是后面每段影片接不起来的根源。两条硬边界：
  **不是首帧来源**（每格只有九分之一像素，裁边会带进相邻格的分隔线；板里坏一格，整板重
  生成会让另外八格一起变，一致性白付），**格数跟随已选定的镜数、版式永不决定镜头数**。
  它作为 `keyframe-prompts.md` 里的一节，不新增产物。

  **记为新增**：不做板的项目行为不变。

- **剧本阶段的场景压力表**（`SCR-12`–`SCR-14`）。新增
  `references/scene-pressure-and-performance.md`：场景功能九格（目的／谁要什么／障碍／
  赌注／战术序列／节拍变化／潜台词／物理任务／谁输了）、战术更换与节拍的可见载体、
  物理任务与「停下手上的活」当重音、距离与地位的可见标记、听与反应的六条判准，以及
  情绪惯性与群戏成波。写作侧的 0–5 自评量表随之进入，但明确**只用于自查**，不进任何
  交付文本，也不单独阻断交付。

- **图片提示词的视觉契约层与能力车道**（`IMG-13`–`IMG-16`）。
  `references/palette-light-and-register.md` 补上此前完全缺席的一层：60/30/10 三层调色
  （真实色名、比例必须可追溯）、技术性的光（主光源／方向／亮暗比／衰减）、材质的
  「名字＋表面处理」、择一不叠的视觉暂存器，以及写实漂移的正向修法。
  `references/capability-lanes.md` 把「这条 prompt 交给哪一种能力」写成六条**能力车道**
  而非工具名：编辑固定从「以原图为底的最小改动」开始，材质重生成不接定点修改，
  需要重建的画面不是编辑，换视角必须逐个物件写出新排布。

- **视频提示词的光学、物理与光线优先锁**（`VID-21`–`VID-27`）。
  `references/optics-lens-language.md` 用**对角视场角＋机位距离＋可观察结果**取代器材参数
  作为主要控制手段，并给出按内容类型（而非按情绪）选择镜头性格的规则、长焦／广角的
  可观察结果堆叠与防漂移锁。`references/physics-lighting-and-first-frame.md` 收拢三类
  「文字上读不出来、只有生成出来才看得见」的失败：首帧占位、物理因果、光线优先，
  并规定多镜切点类型与逐切点必须保住的连续性。`references/performance-projection.md`
  负责把主档案改写进这一镜，含眼神随节拍变化、群戏反应成波与说话时声音身份逐字复用。
  `production-prompt-grammar` 同时补上**分区文本形状**：视频交付文本需要把空间、光学与
  时序切开锁住，图片交付文本不需要——硬统一成一种，两边都会变差。

- **分镜阶段的空间锁与关键帧表演投影**（`SHT-21`–`SHT-24`）。
  `references/spatial-lock.md` 把走位七项写成**可测量**语言（接触优先，其次数字，
  再次相对关系），并要求身体朝向与视线**分开写**——两者的分离常常就是这一镜的戏。
  `references/performance-projection.md` 给出把主档案落进「一个能同时成立的瞬间」的四条
  规则，以及帧内绝对禁止的时间先后词。
