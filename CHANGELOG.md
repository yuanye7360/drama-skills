...
- **场次故事板 previs 层：`shots` → sheet → 每格首帧**（`SHT-21`–`SHT-27`）。
  `$short-drama-storyboard` 在镜头与关键帧之间新增 `storyboard_sheet`：一组镜头画进一张
  多格草图纸，每格是该镜 start beat 的粗略草图，配红/蓝/绿/橙/黄颜色标注承载运动、机位、
  构图、光向与 VFX 意图。整场节奏、走位与摄影一次摊在一张纸上，在花钱渲染前比较修正。
  纸面本体是黑白规划稿，与项目最终画风脱钩——上最终画风材质就越权占了关键帧的活。
  一张 sheet 覆盖一个连续叙事节拍组，不强求恰好一个场景；格与镜头一对一，格顺序即叙事
  先后，每格只投影 start beat。这一层**可省**：简单场次直接从镜头进关键帧，密集调度或
  动作戏才值得先出 sheet。接受后每格投影成对应镜头的一张干净首帧；video 阶段的首帧一律
  取关键帧，不取粗草图 sheet，唯一例外是预算不足时拿 sheet 当动作顺序参考
  （`role=action_sequence`）。派生文本由 `render_sheet_prompts.py` 渲染，不手写。
