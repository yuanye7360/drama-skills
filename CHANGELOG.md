...
- **创作者阅读用的故事板**（`剧集/<EP>/storyboard/storyboard.md`，`SHT-25`）。此前分镜
  阶段唯一的可读产物是 `keyframe-prompts.md`，而它的版式是**生成队列**——一镜一段、
  方便逐条复制去生成，读不出整场的节奏与左右关系。现在同源再渲染一份板：一场一页、
  逐镜一格、带场次时长小计与画面位。两份都是派生文本，不构成第二份事实来源；板带镜号、
  场号与画面位路径，**明确不得整段交给执行端**，否则这些工艺词会被当成画面描述读出来。
  创作台把它显示为「故事板」。**记为新增**：只在创作者需要整场阅读时生成，不生成的项目
  行为不变。
